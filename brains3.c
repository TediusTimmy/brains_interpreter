...
*/
int createThread
   (struct PCB * npar, int npc, int ndp, char * ncmem, int * ns, int nsp)
 {
   struct TCB * c;
   ...
   if (c != NULL)
    {
      ...
      if (nprocs == NULL)
         for (i = 0; i < NUMPROC; i++) c->procs[i] = NOPROC;
      else
         memcpy(c->procs, nprocs, NUMPROC * sizeof(int));
      ...
      if (c->dmem != NULL)
       {
         ...
         if (!createThread(c, npc, ndp, c->dmem, ns, nsp))
          {
...
/*
   ...
   Return:
      ...
      2 Sleep
*/
int doQuanta (int * imem, struct TCB * me, int quanta)
 {
      ...
      switch (curc & IMASK)
       {
         ...
         case ':':
            count = procNum(imem[me->pc]);
            if (count != NOPROC)
               me->par->procs[count] = me->pc + 1;
         ...
         case '&':
            ...
            if (createThread(me->par, me->pc, (me->dp + 1) & DMASK, me->cmem,
                             me->stack, me->sp))
         ...
         case '%':
            me->cmem[me->dp] = 0;
            me->cmem[(me->dp + 1) & DMASK] = 1;
            if (createProcess(me->cmem, me->par->dmem, me->par->procs, me->pc,
         ...
         default:
            curc = procNum(curc);
            if ((curc != NOPROC) && (me->par->procs[curc] != NOPROC))
             {
               if ((imem[me->pc] == ';') || (imem[me->pc] == '$'))
                  me->pc = me->par->procs[curc];
               else if (me->sp == 0)
...
int Compile (int * mimem, FILE * fin, FILE ** useMe, char * tsmem)
 {
   ...
   while (!feof(fin))
    {
      if (createProcess(tsmem, tsmem, NULL, cp, 0, NULL, STACKSIZE))
